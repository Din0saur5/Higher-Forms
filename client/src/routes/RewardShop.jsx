...
const RewardShop = () => {
  ...
  const addToCart = (product) => {
    if (!userData || userData.points === undefined) {
      alert("You must log in to redeem rewards!");
      return;
    }
  ...
  const checkout = () => {
    if (!userData || userData.points === undefined) {
      alert("You must log in to checkout rewards.");
      return;
    }
    ...
    if (userData.points >= totalPoints) {
      userData.points -= totalPoints; // Simulated points update
      setCart([]);
  ...
  return (
    <div className="reward-shop-container bg-black text-white font-roboto min-h-screen p-6">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-lg mt-4">Browse our exclusive rewards! Log in to redeem your points.</p>
        <hr className="mt-4 border-t border-gray-600 mx-auto w-3/4" />
      </div>

      {/* User Points (Only If Logged In) */}
      {userData && userData.points !== undefined && (
        <div className="text-center mb-8">
          <p className="text-xl font-semibold">
            Your Points: <span className="text-gold">{userData.points}</span>
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((product) => (
          <div key={product.id} className="card bg-gray-800 p-4 rounded-lg shadow-lg">
            ...
            >
              {userData ? "Add to Cart" : "Log in to Redeem"}
            </button>
          </div>
        ))}
      </div>

      {/* Cart Section (Only If Logged In) */}
      {userData && userData.points !== undefined ? (
        ...
        <div className="text-center mt-8">
          <p className="text-xl">
            Sign up or log in to start earning points and redeem rewards for exclusive items!
          </p>
