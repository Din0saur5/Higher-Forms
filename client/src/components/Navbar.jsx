    ...
    <>
      <nav className="navbar bg-base-100 px-4 py-3 fixed top-0 left-0 w-full z-50 shadow-md flex justify-between items-center">
        ...
        <div className="navbar-end lg:hidden relative">
          ...
          {/* Mobile Menu */}
          <div
            className={`absolute top-[60px] right-0 w-48 bg-base-100 rounded-lg shadow-lg transition-all duration-300 ease-in-out ${
              click ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-[-10px] pointer-events-none"
            }`}
            style={{ zIndex: 50 }}
          >
        ...
        <div className="navbar-end flex">
          {!userData ? (
            ...
            <>
              <NavLink to="/profile" className="btn btn-primary px-4 py-2 rounded-md hover:bg-primary-focus transition duration-300 mr-2">
                Profile
              </NavLink>
              <button
                onClick={handleLogout}
                className="btn btn-secondary px-4 py-2 rounded-md hover:bg-secondary-focus transition duration-300"
              >
                Logout
              </button>
            </>
